...
export class BookingService {
  async list(user: AppUser): Promise<BookingRecord[]> {
    ...
    return bookingRepository.findManyByBusinessId(
      businessId,
  ...
  async create(
    ...
    if (!vehicle || vehicle.customerId !== customerId)
      throw new AppError('Vehicle was not found for this customer.', HttpStatus.NOT_FOUND);
    const service = input.serviceId
      ? await bookingRepository.findService(businessId, input.serviceId)
      : null;
  ...
  async start(user: AppUser, id: string): Promise<BookingRecord> {
    ...
    if (user.role === 'WORKER' && booking.workerId !== user.id)
      throw new AppError('Booking was not assigned to you.', HttpStatus.FORBIDDEN);
    return bookingRepository.updateStatus({
      id,
  ...
  async cancel(user: AppUser, id: string, reason?: string): Promise<BookingRecord> {
    ...
    return bookingRepository.updateStatus({
      ...
      businessId: booking.businessId,
      status: 'CANCELLED',
      skipReason: this.optText(reason),
    });
  }
  ...
  async rate(
    ...
    if (user.role !== 'CUSTOMER' || booking.customerId !== user.id)
      throw new AppError('Only the customer can rate this booking.', HttpStatus.FORBIDDEN);
  ...
  status(value: unknown): BookingStatus {
    ...
    if (
      !['PENDING', 'ASSIGNED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'SKIPPED'].includes(status)
    )
  ...
  int(value: unknown, field: string): number {
    ...
    if (!Number.isInteger(parsed))
      throw new AppError(field + ' must be an integer.', HttpStatus.BAD_REQUEST);
    return parsed;
  }
  ...
  private async requireBooking(user: AppUser, id: string): Promise<BookingRecord> {
    ...
    if (!booking || (user.role === 'CUSTOMER' && booking.customerId !== user.id))
      throw new AppError('Booking was not found.', HttpStatus.NOT_FOUND);
    return booking;
  }
  ...
  private date(value: string, field: string): Date {
    ...
    if (Number.isNaN(date.getTime()))
      throw new AppError(field + ' must be a valid date.', HttpStatus.BAD_REQUEST);
    return date;
  }

  private requireOwner(user: AppUser): void {
    if (!['OWNER', 'SYSTEM_ADMIN'].includes(user.role))
      throw new AppError('Only owners can perform this action.', HttpStatus.FORBIDDEN);
  }
