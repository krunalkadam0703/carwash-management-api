...
export class ImageService {
  async uploadVehicleImage(user: AppUser, vehicleId: string, file: Express.Multer.File, caption?: string): Promise<VehicleImageRecord> {
    const businessId = this.requireBusinessId(user);
    const vehicle = await imageRepository.findVehicle(businessId, vehicleId);
    if (!vehicle || (user.role === 'CUSTOMER' && vehicle.customerId !== user.id)) throw new AppError('Vehicle was not found.', HttpStatus.NOT_FOUND);

    const stored = await localFileStorageService.saveBuffer('vehicles', file.buffer, file.originalname);
    return imageRepository.createVehicleImage({ vehicleId, imageUrl: stored.publicUrl, caption });
  }

  async uploadServiceImage(user: AppUser, serviceId: string, file: Express.Multer.File, caption?: string, sortOrder?: number): Promise<ServiceImageRecord> {
    this.requireOwner(user);
    const businessId = this.requireBusinessId(user);
    if (!(await imageRepository.findService(businessId, serviceId))) throw new AppError('Service was not found.', HttpStatus.NOT_FOUND);

    const stored = await localFileStorageService.saveBuffer('services', file.buffer, file.originalname);
    return imageRepository.createServiceImage({ serviceId, imageUrl: stored.publicUrl, caption, sortOrder });
  }
  ...
  private requireOwner(user: AppUser): void {
    if (user.role !== 'OWNER') throw new AppError('Only owners can upload service images.', HttpStatus.FORBIDDEN);
  }
