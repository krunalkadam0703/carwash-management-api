...
export class SubscriptionPauseService {
  async list(user: AppUser): Promise<SubscriptionPauseRecord[]> {
    ...
    return subscriptionPauseRepository.findManyByBusinessId(
      businessId,
  ...
  async create(
    user: AppUser,
    input: { subscriptionId: string; startDate: Date; endDate: Date; reason?: string },
  ): Promise<SubscriptionPauseRecord> {
    if (user.role !== 'CUSTOMER')
      throw new AppError('Only customers can pause subscriptions.', HttpStatus.FORBIDDEN);
    const businessId = this.requireBusinessId(user);
    const subscription = await subscriptionPauseRepository.findSubscription(
      businessId,
    ...
    if (subscription.status !== 'ACTIVE')
      throw new AppError('Only active subscriptions can be paused.', HttpStatus.CONFLICT);
    if (input.endDate < input.startDate)
      throw new AppError('endDate must be after startDate.', HttpStatus.BAD_REQUEST);
  ...
  date(value: unknown, field: string): Date {
    ...
    if (Number.isNaN(parsed.getTime()))
      throw new AppError(field + ' must be valid.', HttpStatus.BAD_REQUEST);
