...
const customerListKey = (businessId: string, customerId: string): string =>
  `vehicle:${businessId}:customer:${customerId}`;
  ...
  async findMany(businessId: string, customerId?: string): Promise<VehicleRecord[] | null> {
    try {
      const value = await redisService.get(
        customerId ? customerListKey(businessId, customerId) : ownerListKey(businessId),
      );
