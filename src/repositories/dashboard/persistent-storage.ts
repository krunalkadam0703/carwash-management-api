...
type PaymentDelegate = CountDelegate & {
  aggregate(args: unknown): Promise<{ _sum: { amount?: { toString(): string } | null } }>;
};
type AppDb = {
  user: CountDelegate;
...
export class DashboardPersistentStorageRepository {
  async ownerSummary(businessId: string): Promise<OwnerDashboardSummary> {
    ...
    return {
      ...
      todayWashes,
      unreadComplaints,
      paidRevenue: paidRevenue._sum.amount?.toString() ?? '0',
    };
  }
  ...
  async customerSummary(businessId: string, customerId: string): Promise<CustomerDashboardSummary> {
    ...
    const [
      ...
      db.vehicleSubscription.count({
        where: {
          businessId,
          customerId,
          status: { in: ['REQUESTED', 'APPROVED', 'PAYMENT_PENDING'] },
        },
      }),
